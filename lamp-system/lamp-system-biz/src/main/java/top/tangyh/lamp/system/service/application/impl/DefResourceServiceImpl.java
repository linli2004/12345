...
public class DefResourceServiceImpl extends SuperCacheServiceImpl<DefResourceManager, Long, DefResource> implements DefResourceService {
    ...
    @Override
    public List<DefResource> findResourceListByTenantIdAndApplicationIds(Long tenantId, List<Long> applicationIdList, Collection<String> resourceTypes) {
        return superManager.findResourceListByTenantIdAndApplicationIds(tenantId, applicationIdList, resourceTypes);
    }
    ...
    public List<DefResource> findByIdsAndType(Long tenantId, Long applicationId, Collection<Long> idList, Collection<String> types) {
        return superManager.findByIdsAndType(tenantId, applicationId, idList, types);
    }
    ...
    @Override
    public void clearCache(Long applicationId) {
        cacheOps.del(AllResourceApiCacheKeyBuilder.builder());

        List<DefTenant> defTenants = defTenantManager.listNormal();
        List<CacheKey> tenantResourceKeys = defTenants.stream().map(item -> TenantResourceCacheKeyBuilder.builder(item.getId(), applicationId)).toList();
        cacheOps.del(tenantResourceKeys);

        List<DefResource> resourceList = superManager.list(Wraps.<DefResource>lbQ().eq(DefResource::getApplicationId, applicationId));
        if (CollUtil.isNotEmpty(resourceList)) {
            List<Long> resourceIdList = resourceList.stream().map(DefResource::getId).toList();
            List<CacheKey> resourceKeys = resourceIdList.stream().map(ResourceCacheKeyBuilder::builder).toList();
            cacheOps.del(resourceKeys);


            List<DefResourceApi> resourceApiList = defResourceApiManager.findByResourceId(resourceIdList);
            List<CacheKey> apiKeys = resourceApiList.stream().map(DefResourceApi::getId).map(ResourceApiCacheKeyBuilder::builder).toList();
            cacheOps.del(apiKeys);
        }
    }
