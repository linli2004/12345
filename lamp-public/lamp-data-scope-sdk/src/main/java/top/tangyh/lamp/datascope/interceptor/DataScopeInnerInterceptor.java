...
public class DataScopeInnerInterceptor implements InnerInterceptor {
    ...
    public List<DataFieldProperty> findDataFieldProperty(List<DataFieldProperty> dataFieldProperties) {
        ...
        log.info("path={}, employeeId={}, applicationId={}, tenantId={}", path, employeeId, applicationId, ContextUtil.getTenantId());
    ...
    protected void processSelect(Select select, List<DataFieldProperty> dataFieldPropertyList) {
        ...
        if (!CollectionUtils.isEmpty(withItemsList)) {
            withItemsList.forEach(item -> processSelectBody(item.getSelect(), dataFieldPropertyList));
        }
    }
    ...
    protected void processSelectBody(Select selectBody, List<DataFieldProperty> dataFieldPropertyList) {
        ...
        if (selectBody instanceof PlainSelect sb) {
            ...
            // with 语句
            processSelectBody(parenthesedSelect.getSelect(), dataFieldPropertyList);
    ...
    protected Expression builderExpression(Expression currentExpression, DataFieldProperty dsField) {
        ...
        if (values.size() > 1) {
            ...
            ExpressionList<LongValue> expressionList = new ExpressionList<>(tenantIdList);

            Expression listExpression = new ParenthesedExpressionList<>().withExpressions(expressionList);
